...
-- run this once against every existing database. NOT VALID skips re-checking
-- existing rows (the old constraint allowed internal whitespace), so this
-- cannot fail on existing data; new and updated rows are still enforced.
--
