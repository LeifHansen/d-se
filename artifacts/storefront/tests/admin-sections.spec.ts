...
test.describe("admin spam quarantine", () => {
  test("renders a quarantined item with Forward and Delete actions", async ({
    page,
  }) => {
    const items: QuarantineEntry[] = [
      {
        id: 42,
        name: "Spammy Sam",
    ...
    await page.route(
      ...
      async (route: Route) => {
        ...
        const forwardMatch =
          /\/api\/admin\/contact-quarantine\/(\d+)\/forward$/.exec(
            url.pathname,
          );
        if (forwardMatch && method === "POST") {
          ...
          if (target) target.forwardedAt = "2026-05-15T10:00:00.000Z";
          await route.fulfill({
            status: 200,
            contentType: "application/json",
            body: JSON.stringify({
              id,
              forwardedAt: "2026-05-15T10:00:00.000Z",
            }),
          });
          return;
        }

        const deleteMatch = /\/api\/admin\/contact-quarantine\/(\d+)$/.exec(
          url.pathname,
        );
        ...
        if (
          url.pathname.endsWith("/api/admin/contact-quarantine") &&
          method === "GET"
        ) {
          await route.fulfill({
            status: 200,
    ...
    const row = page.getByTestId("row-quarantine-42");
    await expect(row).toBeVisible();
    await expect(row).toContainText("Spammy Sam");
    await expect(row).toContainText("spammy@example.com");
    await expect(row).toContainText("Cheap deals!!");
    await expect(row).toContainText("too_many_links");

    const forwardBtn = page.getByTestId("button-forward-42");
    const deleteBtn = page.getByTestId("button-delete-42");
    await expect(forwardBtn).toBeVisible();
    await expect(deleteBtn).toBeVisible();

    await forwardBtn.click();
    await expect.poll(() => forwarded).toContain(42);
    await expect(page.getByTestId("text-forwarded-42")).toBeVisible();

    page.once("dialog", (dialog) => {
      void dialog.accept();
    });
    await deleteBtn.click();
    await expect.poll(() => deleted).toContain(42);
    await expect(page.getByTestId("row-quarantine-42")).toHaveCount(0);
