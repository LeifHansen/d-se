...
export function AgeGate() {
  ...
  return (
    ...
    >
      <div className="grid w-full max-w-3xl overflow-hidden rounded-3xl bg-white text-ink shadow-2xl md:grid-cols-[1.1fr_0.9fr]">
        <div className="grain flex flex-col items-center justify-center gap-5 p-8 text-center md:p-10">
          <Logo variant="stacked" tone="ink" />
          {denied ? (
            <>
              ...
              <p className="max-w-xs text-sm text-graphite">
                You must be 21 or older to enter this site. Please come back
                when you meet the age requirement.
              </p>
            </>
          ) : (
            <>
              <p className="eyebrow text-turquoise-deep">Please confirm your age</p>
              <h2
                id="age-gate-title"
                className="font-display text-5xl leading-[0.92]"
              >
                Are you at least{" "}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  ref={yesButtonRef}
                  onClick={confirm}
                  className="cta cta-pink min-w-[7.5rem]"
                ...
                <button
                  type="button"
                  onClick={deny}
                  className="cta cta-outline min-w-[7.5rem]"
                  data-testid="age-gate-no"
                >
                  No
                </button>
              </div>
              <p className="max-w-xs text-[11px] leading-relaxed text-graphite">
        ...
        <div className="hidden md:block">
          <ImagePlaceholder
            label="Age gate visual"
            hint="Replace · 900 × 1200"
            slug="age-gate"
            aspect="auto"
            className="h-full rounded-none border-0 border-l-2 border-dashed border-silver"
          />
        </div>
      </div>
    </div>
